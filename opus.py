...
class CodeWriter:
    ...
    def writeProposal(self):
        ...
        self.writeChapters(proposal=True)
        self.writeReferences()
        self.writeEnding()
    ...
    def writeHeader(self):
        ...
        self.writeLine("\\graphicspath{ {./images/} }")
    ...
    def writeProposalCover(self):
        ...
        self.writeLine("")
        self.writeLine("ภาควิชาวิศวกรรมคอมพิวเตอร์ คณะวิศวกรรมศาสตร์ศรีราชา\\\\")
    ...
    def writeReportMainCover(self):
        ...
        self.writeLine("}")
        self.writeLine("\\vfill")
        self.writeLine("{\\fontsize{20pt}{22pt}\\selectfont\\bfseries ภาควิชาวิศวกรรมคอมพิวเตอร์ คณะวิศวกรรมศาสตร์ศรีราชา\\\\")
        self.writeLine("มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตศรีราชา\\\\")
        self.writeLine("พ.ศ. \\thisYear}\\\\")
        self.writeLine("\\end{center}")
        self.writeLine("")
    ...
    def writeApprovalForm(self, proposal=False):
        information = self.info
        self.writeLine("%%%%%%%%%%%%%%%%%%%% Approval Form %%%%%%%%%%%%%%%%%%%%%")
        self.writeLine("\\newpage")
        self.writeLine("\\begin{center}")
        if proposal:
            self.writeLine("\\textbf{ข้อเสนอโครงงานวิศวกรรมคอมพิวเตอร์}")
        else:
            self.writeLine("\\textbf{ปริญญาวิศวกรรมศาสตรบัณฑิต (วิศวกรรมคอมพิวเตอร์)}")
        self.writeLine("")
        self.writeLine("\\textbf{ภาควิชาวิศกรรมคอมพิวเตอร์ \\hfill คณะวิศวกรรมศาสตร์ศรีราชา}")
        self.writeLine("\\end{center}")
        self.writeLine("\\vfill")
        ...
        if not proposal:
            ...
            self.writeLine("& \\textbf{ภาควิชาวิศวกรรมคอมพิวเตอร์ คณะวิศวกรรมศาสตร์ศรีราชา}\\\\")
    ...
    def writeAbstractTH(self):
        ...
        self.write("ปริญญาวิศวกรรมศาสตรบัณฑิต (วิศวกรรมคอมพิวเตอร์) ภาควิชาวิศวกรรมคอมพิวเตอร์ คณะวิศวกรรมศาสตร์ศรีราชา, ")
    ...
    def writeChapters(self, proposal=False):
        information = self.info
        ...
        for chapter in information["CHAPTER"]:
            if chapterNumber > 3 and proposal:
                break
    ...
    def executeCommand(self):
        ...
        if self.state[-1] == "math":
            ...
            if self.command.startswith("title"):
                ...
                if label != "":
                    result += "\\label{" + label + "}\n"
            elif self.command.startswith("paragraph"):
                section = self.command[7:].strip()
                label = ""
                if section[0] == "(":
                    endLabel = section.find(")")
                ...
                if section[0] == ":":
                    nameTranslator = TexTranslator(self.info)
                    name = nameTranslator.translate(section[1:])
                    result = "\\paragraph{" + name + "}\n"
                if label != "":
                    result += "\\label{" + label + "}\n"
            elif self.command.startswith("subparagraph"):
                section = self.command[10:].strip()
                label = ""
                if section[0] == "(":
                    endLabel = section.find(")")
                ...
                for arg in args[1:]:
                    ...
                    if arg.startswith("math"):
                        ...
                        arg = arg[5:].strip()
                        if arg.startswith("=") and arg[1:].strip() == "false":
                            frame = False
